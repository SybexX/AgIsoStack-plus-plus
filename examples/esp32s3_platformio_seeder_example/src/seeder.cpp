...
bool Seeder::initialize(const std::string &interfaceName)
{
	...
	twai_general_config_t twaiConfig = TWAI_GENERAL_CONFIG_DEFAULT(GPIO_NUM_15, GPIO_NUM_16, TWAI_MODE_NORMAL);
	...
	const std::vector<isobus::NAMEFilter> tcNameFilters = {
		filterTaskController,
		filterTaskControllerInstance,
		filterTaskControllerIndustryGroup,
		filterTaskControllerDeviceClass,
	};
	const std::vector<isobus::NAMEFilter> vtNameFilters = {
		filterVirtualTerminal,
	};
