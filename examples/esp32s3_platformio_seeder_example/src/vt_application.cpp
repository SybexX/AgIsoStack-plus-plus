...
{
	objectPool = isobus::IOPFileInterface::read_iop_file("/iop/object_pool.iop");

	if (objectPool.empty())
	{
		std::cout << "Failed to load object pool from object_pool.iop" << std::endl;
		return false;
	}
	std::cout << "Loaded object pool from object_pool.iop" << std::endl;
	...
	if (isobus::VirtualTerminalClient::KeyActivationCode::ButtonUnlatchedOrReleased == event.keyEvent)
	{
		switch (event.objectID)
		{
			...
			case autoManualToggle_Button:
			{
				...
				TCClientInterface.on_value_changed_trigger(static_cast<std::uint16_t>(SectionControlImplementSimulator::ImplementDDOPElementNumbers::BoomElement), static_cast<std::uint16_t>(isobus::DataDescriptionIndex::RequestDefaultProcessData));
			}
			break;
...
void SeederVtApplication::process_new_speed(SpeedSources source, std::uint32_t speed)
{
	...
	else if ((SpeedSources::WheelBased == source) && (0 == speedMessages.get_number_received_machine_selected_speed_sources()) && (0 == speedMessages.get_number_received_ground_based_speed_sources()))
	{
...
void SeederVtApplication::update()
{
	...
	if (isobus::SystemTiming::time_expired_ms(slowUpdateTimestamp_ms, 1000))
	{
		...
		if ((0 == speedMessages.get_number_received_machine_selected_speed_sources()) && (0 == speedMessages.get_number_received_ground_based_speed_sources()) && (0 == speedMessages.get_number_received_wheel_based_speed_sources()))
		{
...
bool SeederVtApplication::get_is_object_shown(std::uint16_t objectID) const
{
	...
	switch (objectID)
	{
		...
		{
			retVal = ((VTClientUpdateHelper.get_active_mask() == statisticsRunscreen_DataMask) && (VTClientUpdateHelper.get_numeric_value(selectedStatisticsContainer_ObjPtr) == canStatistics_Container));
		}
		break;
		...
		{
			retVal = ((VTClientUpdateHelper.get_active_mask() == statisticsRunscreen_DataMask) && (VTClientUpdateHelper.get_numeric_value(selectedStatisticsContainer_ObjPtr) == utStatistics_Container));
		}
		break;
		...
		{
			retVal = ((VTClientUpdateHelper.get_active_mask() == statisticsRunscreen_DataMask) && (VTClientUpdateHelper.get_numeric_value(selectedStatisticsContainer_ObjPtr) == tcStatistics_Container));
		}
		break;
		...
		case alarm_SKeyMask:
		{
			retVal = ((VTClientUpdateHelper.get_active_mask() == noSpeed_AlarmMask) || (VTClientUpdateHelper.get_active_mask() == noTaskController_AlarmMask));
		}
		break;
...
SeederVtApplication::Alarm::Alarm(std::uint32_t activationDelay_ms) : activationDelay_ms(activationDelay_ms)
{
}

bool SeederVtApplication::Alarm::is_active() const
{
	return (!acknowledged) && (timestampTriggered_ms != 0) && isobus::SystemTiming::time_expired_ms(timestampTriggered_ms, activationDelay_ms);
}
